...
public class AutomaticGunScriptLPFP : MonoBehaviour {
    ...
    [SerializeField] OnlineFPS.FPSController owner;
    ...
    public void SimulateFire(Vector3 bulletSpawnPoint, Quaternion bulletRotation)
    {
        anim.Play("Fire", 0, 0f);
        ...
        bullet.transform.forward * bulletForce * 10;
	...
	private void Update () {
        ...
        if (!owner.IsAlive && !OnlineFPS.NetworkRoomManager.IsInit)
            return;
        ...
        if (photonView.IsMine)
        {
            ...
            if (false && Input.GetKeyDown(KeyCode.Q) && !isInspecting)
            {
                anim.Play("Knife Attack 1", 0, 0f);
            }
            //Play knife attack 2 animation when F key is pressed
            if (false && Input.GetKeyDown(KeyCode.F) && !isInspecting)
            {
                anim.Play("Knife Attack 2", 0, 0f);
            }

            //Throw grenade when pressing G key
            if (false && Input.GetKeyDown(KeyCode.G) && !isInspecting)
            {
            ...
            if (false && Input.GetKeyDown(KeyCode.E) && !hasBeenHolstered)
            {
                holstered = true;
            ...
            else if (false && Input.GetKeyDown(KeyCode.E) && hasBeenHolstered)
            {
                holstered = false;
